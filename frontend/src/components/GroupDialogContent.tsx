...
function MemberTaskAccordion({ userId, name, avatarUrl, isYou, tasks }: any) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div 
        className="flex items-center justify-between cursor-pointer p-1 -mx-1 rounded-lg hover:bg-[#f1f5f9] transition-colors"
...
export function GroupDialogContent() {
    ...
    return (
      <div className="flex flex-col gap-4 font-['Plus_Jakarta_Sans',sans-serif]">
        <div className="flex items-center gap-2 mb-2">
          <IconButton onClick={() => setView("list")} size="small"><ArrowBackIcon /></IconButton>
        ...
        <form onSubmit={handleCreateGroup} className="flex flex-col gap-4">
          <input
            ...
            className="w-full border border-[#e8ede8] rounded-xl p-3 text-[14px] outline-none text-[#0f1f10] bg-[#f8faf8] font-bold focus:border-[#13ec37]"
            autoFocus
          />
          <button
            type="submit"
            disabled={!groupName.trim() || createGroupMutation.isPending}
            className="bg-[#13ec37] text-[#0f1f10] font-bold py-3 rounded-xl shadow-[0_4px_16px_rgba(19,236,55,0.25)] transition-all active:scale-95 disabled:opacity-50"
          >
  ...
  if (view === "detail" && selectedGroupId) {
    ...
    return (
      <div className="flex flex-col gap-4 font-['Plus_Jakarta_Sans',sans-serif] max-h-[70vh] overflow-y-auto pr-2 pb-2">
        <div className="flex items-center gap-2 mb-1 sticky top-0 bg-white z-10 py-2">
          <IconButton onClick={() => { setView("list"); setSelectedGroupId(null); }} size="small"><ArrowBackIcon /></IconButton>
        ...
        <div className="bg-[#13ec37]/10 rounded-xl p-4 border border-[#13ec37]/30 shrink-0">
          ...
          <div className="h-[8px] bg-white/50 rounded-full overflow-hidden">
            <div className="h-full bg-[#13ec37] rounded-full transition-all duration-700" style={{ width: `${teamProgress}%` }} />
          </div>
        </div>
                  ...
                  <div key={userId} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 flex flex-col justify-center">
                      ...
                      <div className="h-[6px] bg-[#f1f5f9] rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-700 ${mPct === 100 ? 'bg-[#13ec37]' : isYou ? 'bg-[#0fbf2c]' : 'bg-[#94a3b8]'}`} 
                          style={{ width: `${mPct}%` }} 
                        />
                      </div>
                    </div>
        ...
        <div>
          ...
          <form onSubmit={handleAddMember} className="flex gap-2 mb-3">
            <select
              ...
              className="flex-1 border border-[#e8ede8] rounded-lg p-2 text-[13px] outline-none bg-[#f8faf8] font-bold text-[#0f1f10] focus:border-[#13ec37]"
            >
            ...
            <button 
              ...
              className="bg-[#0f1f10] text-white px-4 rounded-lg text-[12px] font-bold transition-all active:scale-95 disabled:opacity-50"
            >
              追加
            </button>
          </form>
          ...
          {isLoadingMembers ? (
            ...
            <div className="flex flex-col gap-2">
              {members?.map((member: any) => {
                ...
                return (
                  <div key={member.id} className="flex justify-between items-center p-2 rounded-lg bg-white border border-[#e8ede8]">
                    <div className="flex items-center gap-2">
                      {avatarUrl ? (
    ...
    <div className="flex flex-col gap-4 font-['Plus_Jakarta_Sans',sans-serif]">
      <div className="flex justify-between items-center mb-2">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[#0fbf2c] font-bold text-[13px] bg-[#13ec37]/10 px-3 py-1.5 rounded-full transition-all hover:bg-[#13ec37]/20 border-none cursor-pointer active:scale-95"
        >
        ...
        <div className="flex flex-col gap-3">
          {groups.map((group: any) => (
            <div
              ...
              className="flex items-center gap-3 p-3 rounded-xl bg-[#f8faf8] border border-[#e8ede8] cursor-pointer hover:border-[#13ec37]/50 hover:bg-[#13ec37]/5 transition-all"
            >
